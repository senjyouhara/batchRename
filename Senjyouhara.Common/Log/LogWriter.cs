...
#region Init
        ...
        private void InitArchiveStore()
        {
            ...
            var files = Directory.GetFiles(PathCombine(_basePath, _rootFolder)).Where(v=> !v.Contains(_historyArchiveFolder)).Select(v=> PathCombine(v)).ToList();
        ...
        private void InitCurrentArchiveIndex()
        {
            ...
            string[] fileArr = Directory.GetFiles(_currentStream.CurrentLogFileDir, _currentStream.CurrentLogFileName + "*");
...
#region 拼接日志内容
        ...
        private static string CreateLogString(LogType logType, string log)
        {
            ...
            var line = st.GetFrame(4).GetFileLineNumber();
            System.Reflection.MethodBase mb = st.GetFrame(4).GetMethod();
            ...
            str = mb.DeclaringType.FullName.Replace("+<>c", "");
...
#region UpdateCurrentStream
        ...
        private void UpdateCurrentStream()
        {
            try
            {
                ...
                _currentStream.CurrentLogFilePath = PathCombine(_currentStream.CurrentLogFileDir, _currentStream.CurrentLogFileName + ".txt");
        ...
        private void ListenSaveLogAsync(CancellationToken cancellationToken)
        {
            Task.Factory.StartNew(() =>
            {
                ...
                while (!cancellationToken.IsCancellationRequested)//如果没有取消线程，则一直监听执行写LOG
                {
                    ...
                    if (logMsgQueue.Count >= 10 || (logMsgQueue.Count > 0 && (DateTime.Now - lastSaveLogTime).TotalSeconds > 30))
                    {
